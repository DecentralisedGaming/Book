...
The Xaya team is currently building [Taurion](https://taurion.io/) which is an MMO-scale RTS game. The in-game currency (vCHI) can be freely exchange to the native chain token (that can be earned via mining). In addition to this virtual currency, vCHI, there are the in-game resources which are inflationary in nature. Although the game loop encourages harvested resources to be crafted which transforms them into a fewer number of higher value items.
